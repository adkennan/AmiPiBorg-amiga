...
APTR APB_CreateStatCollector(APTR memPool) {
...
VOID APB_FreeStatCollector(APTR statCollector) {
...
struct StatNode *APB_FindStatNode(APTR ctx, UWORD statType) {
    ...
    if( statType >= sc->sc_StatTypeMean ) {
        ...
        for( sn = (struct StatNode *)STAT_LIST(sc)->lh_Tail;
             sn->sn_Node.ln_Pred;
             sn = (struct StatNode *)sn->sn_Node.ln_Pred ) {
        ...
        for( sn = (struct StatNode *)STAT_LIST(sc)->lh_Head;
             sn->sn_Node.ln_Succ;
             sn = (struct StatNode *)sn->sn_Node.ln_Succ) {
            if( sn->sn_Node.ln_Pri == statType ) {
                return sn;
            }
        }
