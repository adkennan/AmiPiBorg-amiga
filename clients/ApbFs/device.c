...
VOID FS_SendReq(struct ApbFs *fs)
{
    ...
    fs->fs_Writer->r_Data = ((BYTE *)req) + offsetof(struct FsRequest, r_Self);
    fs->fs_Writer->r_Length = sizeof(struct FsRequest) + req->r_Length - offsetof(struct FsRequest, r_Self);
...
VOID FS_ReceiveResponse(struct ApbFs *fs)
{
...
VOID FS_ReceiveFromRemote(struct ApbFs *fs)
{
    struct Message *msg;
...
VOID FS_Run(struct ApbFs *device) 
{
    ...
    if( vol = FS_CreateVolume(0, "APB0") ) {
        AddTail(VOL_LIST(device), (struct Node *)vol);
        device->fs_VolSigBits |= PORT_BIT(vol->v_Port);
    }

    while( device->fs_Status != ST_STOPPING ) {      

        sig = Wait(PORT_BIT(device->fs_RemotePort) |
                   device->fs_VolSigBits | 
                   SIGBREAKF_CTRL_C);

        for( vol = (struct Volume *)VOL_LIST(device)->lh_Head;
             vol->v_Node.mln_Succ;
             vol = (struct Volume *)vol->v_Node.mln_Succ ) {
