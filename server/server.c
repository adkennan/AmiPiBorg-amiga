...
#define ORDIE(msg) else { LOG0(srv->srv_Ctx, LOG_ERROR, msg); }

BOOL APB_SrvCreateTimer(
    struct Server *srv)
{
    LOG0(srv->srv_Ctx, LOG_DEBUG, "Create Timer Port");

    if(srv->srv_TimerPort = CreateMsgPort()) {

        LOG0(srv->srv_Ctx, LOG_DEBUG, "Create Timer Request");

        if(srv->srv_TimerReq = (struct timerequest *) CreateExtIO(srv->srv_TimerPort, sizeof(struct timerequest))) {

            LOG0(srv->srv_Ctx, LOG_DEBUG, "Open Timer Device");
...
BOOL APB_SrvInitServer(
    APTR mp,
    struct Server * srv)
{
    struct Config *cfg;

    if(srv->srv_Ctx = APB_CreateContext(mp, Output(), LOG_INFO)) {

        if(cfg = APB_GetConfig(srv->srv_Ctx)) {

            LOG0(srv->srv_Ctx, LOG_DEBUG, "Create Server");
            ...
            LOG0(srv->srv_Ctx, LOG_DEBUG, "Create Packet Writer");

            if(srv->srv_PacketWriter = APB_CreatePacketWriter(srv->srv_Ctx)) {

                LOG0(srv->srv_Ctx, LOG_DEBUG, "Create Packet Reader");

                if(srv->srv_PacketReader = APB_CreatePacketReader(srv->srv_Ctx)) {
        ...
        ORDIE("Failed to read config")
    }

    return FALSE;
}

BOOL APB_SrvInitRemote(
    struct Server * srv)
{
    LOG1(srv->srv_Ctx, LOG_DEBUG, "Open Remote Library %s", srv->srv_Config->cf_RemoteName);

    if(RemoteBase = OpenLibrary(srv->srv_Config->cf_RemoteName, 1)) {

        LOG0(srv->srv_Ctx, LOG_DEBUG, "Create Remote");

        if(srv->srv_Remote = REM_CreateRemote()) {

            LOG0(srv->srv_Ctx, LOG_DEBUG, "Configure Remote");
    ...
    VOID)
{
    ...
    if(mp = CreatePool(MEMF_ANY, 4096, 2048)) {
        ...
        if(srv = AllocPooled(mp, sizeof(struct Server))) {

            srv->srv_MemPool = mp;

            if(APB_SrvInitServer(mp, srv) && APB_SrvInitRemote(srv) && APB_SrvCreateTimer(srv)) {

                LOG0(srv->srv_Ctx, LOG_DEBUG, "Server Created");
                return srv;
            }
        }
    ...
    Server server)
{
    ...
    if(srv->srv_TimerReq) {
        ...
        LOG0(srv->srv_Ctx, LOG_DEBUG, "Close Timer");
    ...
    while(!IsListEmpty(CONNECTIONS(srv))) {
        ...
        LOG1(srv->srv_Ctx, LOG_INFO, "Destroy Connection %d", APB_GetId(cnn));
        APB_DestroyConnection(cnn);
    }

    if(RemoteBase) {

        LOG0(srv->srv_Ctx, LOG_DEBUG, "Close Remote");
    ...
    if(srv->srv_PacketReader) {
        LOG0(srv->srv_Ctx, LOG_DEBUG, "Destroy Packet Reader");
        APB_DestroyPacketReader(srv->srv_PacketReader);
    }

    if(srv->srv_PacketWriter) {
        LOG0(srv->srv_Ctx, LOG_DEBUG, "Destroy Packet Writer");
        APB_DestroyPacketWriter(srv->srv_PacketWriter);
    }

    LOG0(srv->srv_Ctx, LOG_DEBUG, "Destroy Server");

    APB_FreeConfig(srv->srv_Config);

    mp = srv->srv_MemPool;

    LOG0(srv->srv_Ctx, LOG_DEBUG, "Goodbye Cruel World");

    APB_FreeContext(srv->srv_Ctx);

    if(mp) {
        FreePooled(mp, srv, sizeof(struct Server));

        DeletePool(mp);
    } else {
        FreeMem(srv, sizeof(struct Server));
    }
}
...
VOID APB_EnableTimer(
    ...
    ULONG timeoutMillis)
{
    LOG1(srv->srv_Ctx, LOG_TRACE, "Set timer to %dms", timeoutMillis);
    ...
    struct Server *srv)
{
    ...
    LOG0(srv->srv_Ctx, LOG_TRACE, "Write Buffer");
    ...
    struct Server * srv)
{
    ...
    LOG0(srv->srv_Ctx, LOG_TRACE, "Ping");
    ...
    UWORD errorCode)
{
    ...
    LOG2(srv->srv_Ctx, LOG_ERROR, "Error: connId = %d, errorCode = %d", connId, errorCode);
    ...
    struct InPacket * ip)
{
    ...
    LOG1(srv->srv_Ctx, LOG_INFO, "Resend request for packet %d", packId);
    ...
    struct InPacket *ip)
{
    ...
    if(!(p = APB_AllocMem(srv->srv_Ctx, size))) {
        LOG1(srv->srv_Ctx, LOG_ERROR, "Can't allocate %d bytes for hello packet", size);
        return;
    }

    APB_InPacketToBuffer(ip, p);

    LOG1(srv->srv_Ctx, LOG_INFO, "Server version %d", p->ph_Version);

    LOG1(srv->srv_Ctx, LOG_INFO, "%d Handlers Available:", p->ph_HandlerCount);
    if(p->ph_HandlerCount > 0) {
        h = (struct HandlerDesc *) (p + 1);

        for(ix = 0; ix < p->ph_HandlerCount; ix++) {

            LOG2(srv->srv_Ctx, LOG_INFO, "\t%d: %s", h->hd_HandlerId, h->hd_HandlerName);

            h++;
        }
    }

    APB_FreeMem(srv->srv_Ctx, p, size);
}
    ...
    struct InPacket *ip)
{
    ...
    case PT_HELLO:
        LOG0(srv->srv_Ctx, LOG_DEBUG, "Hello Received");
    ...
    case PT_GOODBYE:
        LOG0(srv->srv_Ctx, LOG_DEBUG, "Goodbye Received");
        srv->srv_State = SS_DISCONNECTED;
        break;

    case PT_PONG:
        LOG0(srv->srv_Ctx, LOG_TRACE, "Ping Received");
        srv->srv_MissedPingCount = 0;
        break;

    case PT_RESEND:
        LOG0(srv->srv_Ctx, LOG_DEBUG, "Resend Request Received");
        APB_HandleResendRequest(srv, ip);
        break;

    default:
        LOG1(srv->srv_Ctx, LOG_ERROR, "Unknown control packet %d", ip->ip_Type);
        break;
    }
    ...
    UWORD currPackId)
{
    ...
    while(packId < currPackId) {
        ...
        LOG1(srv->srv_Ctx, LOG_DEBUG, "Requesting Resend of packet %d", packId);
    ...
    struct InPacket *ip)
{
    ...
    LOG1(srv->srv_Ctx, LOG_TRACE, "Received packet %d", ip->ip_PackId);
    ...
    if(!(ip->ip_Flags & PF_RESEND)) {
        if(ip->ip_PackId - 1 > srv->srv_LastInPackId) {
            LOG2(srv->srv_Ctx, LOG_DEBUG, "Expecting packet %d, got %d", srv->srv_LastInPackId + 1, ip->ip_PackId);
        ...
        LOG1(srv->srv_Ctx, LOG_DEBUG, "Received resent packet %d", ip->ip_PackId);
    }
    ...
    if(ip->ip_ConnId == DEFAULT_CONNECTION) {
        ...
        if(cnn == NULL) {
            ...
            LOG1(srv->srv_Ctx, LOG_DEBUG, "Packet received for unknown connection %d", ip->ip_ConnId);
    ...
    struct Server *srv)
{
    ...
    if(srv->srv_ReadBuffer == NULL || BUF_FULL(srv->srv_ReadBuffer)) {
        if(!(buf = APB_AllocateBuffer(srv->srv_Ctx))) {
            LOG0(srv->srv_Ctx, LOG_ERROR, "Unable to allocate read buffer");
        } else {
    ...
    LOG1(srv->srv_Ctx, LOG_TRACE, "Read up to %d bytes", BUF_AVAIL(srv->srv_ReadBuffer));
    ...
    struct Server * srv)
{
    ...
    LOG1(srv->srv_Ctx, LOG_DEBUG, "Read %d bytes", bytesRead);
    ...
    struct APBRequest *req)
{
    ...
    switch (req->r_Type) {
    case APB_RT_OPEN:
        cnn = APB_CreateConnection(srv->srv_Ctx, CONNECTIONS(srv), srv->srv_NextConnId++, srv->srv_PacketWriter);
        APB_HandleClientRequest(cnn, req);
        replyMsg = FALSE;
        break;

    case APB_RT_SRV_QUIT:
        LOG0(srv->srv_Ctx, LOG_INFO, "Quit Received");
        srv->srv_State = SS_DISCONNECTED;
        break;

    case APB_RT_SRV_LOG_LEVEL:
        ll = *(UWORD *) req->r_Data;
        LOG1(srv->srv_Ctx, LOG_INFO, "Set Log Level to %d", ll);
        APB_SetLogLevel(srv->srv_Ctx, ll);
        break;
...
//        APB_CopyLog(req->r_Data, req->r_Length);
    ...
    default:
        LOG1(srv->srv_Ctx, LOG_ERROR, "Unknown packet type %d receieved from client.", req->r_Type);
        req->r_State = APB_RS_FAILED;
        break;
    }
    ...
    struct Server *srv)
{
    ...
    while(req = (struct APBRequest *) GetMsg(srv->srv_ClientPort)) {
        ...
        LOG2(srv->srv_Ctx, LOG_TRACE, "Got request %x, conn %d", req, req->r_ConnId);
        ...
        if(req->r_ConnId == DEFAULT_CONNECTION) {
            ...
            if(cnn == NULL) {
                ...
                LOG1(srv->srv_Ctx, LOG_ERROR, "Received message from invalid connection %d", req->r_ConnId);
    ...
    struct Server *srv)
{
    ...
    case SS_WAIT_FOR_CONNECT:
        LOG0(srv->srv_Ctx, LOG_ERROR, "Failed to connect.");
    ...
    case SS_CONNECTED:
        if(srv->srv_PingTimeoutTicks == 1) {
            ...
            if(srv->srv_MissedPingCount > MAX_MISSED_PINGS) {
                LOG0(srv->srv_Ctx, LOG_ERROR, "Remote connection timeout.");
    ...
    struct Server *srv)
{
    ...
    LOG1(srv->srv_Ctx, LOG_TRACE, "Wait for sigs 0x%x", sig);

    sig = Wait(sig);

    LOG1(srv->srv_Ctx, LOG_TRACE, "Received sig 0x%x", sig);

    if(sig & SIGBREAKF_CTRL_C) {
        LOG0(srv->srv_Ctx, LOG_INFO, "Break!");
        srv->srv_State = SS_DISCONNECTED;
    }
...
BOOL APB_OpenClient(
    struct Server *srv)
{
    LOG0(srv->srv_Ctx, LOG_DEBUG, "Open Client Port");
    ...
    struct Server * srv)
{
    ...
    LOG0(srv->srv_Ctx, LOG_DEBUG, "Send Init");
    LOG1(srv->srv_Ctx, LOG_DEBUG, "Version = %d", AMIPIBORG_VERSION);
    LOG1(srv->srv_Ctx, LOG_DEBUG, "MaxPacketSize = %d", BUFFER_SIZE);
    ...
    struct Server * srv)
{
    ...
    LOG0(srv->srv_Ctx, LOG_DEBUG, "Send Shutdown");
    ...
    Server server)
{
    ...
    LOG0(srv->srv_Ctx, LOG_INFO, "Starting");
    ...
    while(srv->srv_State != SS_STOPPED) {
        ...
        case SS_STARTING:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Starting");
        ...
        case SS_OPENING_REMOTE:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Opening Remote");
        ...
        case SS_OPENING_CLIENT:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Opening Client");
        ...
        case SS_CONNECTING:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Connecting");
        ...
        case SS_WAIT_FOR_CONNECT:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Wait For Connect");

            APB_WaitForMessage(srv);
            break;

        case SS_CONNECT_FAILED:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Connect Failed");
        ...
        case SS_CONNECTION_LOST:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Connection Lost");

            srv->srv_State = SS_CONNECTING;
            break;

        case SS_DISCONNECTING:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Disconnecting");
        ...
        case SS_DISCONNECTED:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Disconnected");

            srv->srv_State = SS_STOPPING;
            break;

        case SS_STOPPING:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Stopping");

            srv->srv_State = SS_CLOSING_CLIENT;
            break;

        case SS_CLOSING_CLIENT:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Closing Client");
        ...
        case SS_CLOSING_REMOTE:
            LOG0(srv->srv_Ctx, LOG_TRACE, "Closing Remote");
    ...
    LOG0(srv->srv_Ctx, LOG_TRACE, "Stopped");
}
